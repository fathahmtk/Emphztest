    ...
    <>
      ...
      {!isOpen && (
        <button
          ...
          className="fixed bottom-6 left-6 z-40 bg-emphz-orange text-white w-14 h-14 md:w-16 md:h-16 rounded-full shadow-lg hover:scale-110 transition-transform flex items-center justify-center group animate-fade-up ring-4 ring-white/10"
          ...
          <header className="flex items-center justify-between p-4 border-b border-white/10 flex-shrink-0 bg-white/5 backdrop-blur-md">
            <div className="flex items-center">
              <div className="relative">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-emphz-orange to-blue-600 flex items-center justify-center shadow-lg">
              ...
              <div className="ml-3">
                ...
                <p className="text-[10px] text-emphz-orange uppercase tracking-wider font-bold">GRP Specialist</p>
              </div>
            </div>
            <button
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-[#0B1120] to-[#050A14]" role="log" aria-live="polite">
            {messages.map((msg, i) => (
              <div key={i} className={`flex items-end gap-2 ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.sender === 'agent' && (
                  <div className="w-6 h-6 rounded-full bg-gradient-to-br from-emphz-orange to-blue-600 flex-shrink-0 flex items-center justify-center text-[10px] text-white font-bold">
                    P
                  </div>
                )}
          ...
          <form onSubmit={handleSendMessage} className="p-3 md:p-4 border-t border-white/10 flex-shrink-0 bg-white/5 backdrop-blur-md">
            <div className="relative">
              <input
                ...
                className="w-full bg-black/40 border border-white/10 rounded-full pl-4 pr-12 py-3 text-white placeholder-gray-500 focus:ring-2 focus:ring-emphz-orange focus:border-emphz-orange outline-none text-sm font-sans transition-all"
                aria-label="Your message"
              />
              <button
                type="submit"
                className="absolute right-1.5 top-1/2 -translate-y-1/2 w-9 h-9 bg-emphz-orange text-white rounded-full flex items-center justify-center hover:bg-[#00D4DE] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
